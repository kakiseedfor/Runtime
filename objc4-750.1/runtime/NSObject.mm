...
storeWeak(id *location, objc_object *newObj)
{
 ...
 retry:
    ...
    if (haveNew  &&  newObj) {
        ...
        /*
         +initialize是一个可多线程调用的方法；
         这里为了防止在 +initialize 也在使用弱引用，需要将锁释放，重新走流程。
         */
...
objc_object::sidetable_addExtraRC_nolock(size_t delta_rc)
{
    ...
    /*
     从 table.refcnts 获取的值还不是直接的引用计数，还存有 2bit 标志位。
     */
    size_t& refcntStorage = table.refcnts[this];
    size_t oldRefcnt = refcntStorage;
    ...
    /*
     delta_rc << SIDE_TABLE_RC_SHIFT 对应上面 2bit 的标志位
     */
    uintptr_t carry;
    size_t newRefcnt = 
    ...
    if (carry) {
        /*
         因为 (oldRefcnt & SIDE_TABLE_DEALLOCATING) = 0，(oldRefcnt & SIDE_TABLE_WEAKLY_REFERENCED) = 0
         所以(oldRefcnt & SIDE_TABLE_FLAG_MASK) == 0
         */
        refcntStorage =
