...
static void append_referrer(weak_entry_t *entry, objc_object **new_referrer)
{
    //正常情况weak_entry_t.inline_referrers够用的情况。
    if (! entry->out_of_line()) {
        ...
        //weak_entry_t.inline_referrers不够用，需要扩容。
        ...
        entry->out_of_line_ness = REFERRERS_OUT_OF_LINE;    //设置标示，表明这个已扩容过的。
    ...
    //判断已扩容的使用情况，已使用很多了，则需要h重新扩容。
    if (entry->num_refs >= TABLE_SIZE(entry) * 3/4) {
        return grow_refs_and_insert(entry, new_referrer);
    }
    //根据对象指针的指针，获取对应的Hash 存储地址。
...
{
    objc_object *referent = (objc_object *)referent_id; //实例对象的指针
    objc_object **referrer = (objc_object **)referrer_id;   //指向该实例对象的指针的指针
    ...
    //根据对象的指针找到对应的弱引用存储入口 weak_entry_t
    // now remember it and where it is being stored
    weak_entry_t *entry;
    if ((entry = weak_entry_for_referent(weak_table, referent))) {
        append_referrer(entry, referrer);   //找到则插入对应的 weak_entry_t
    } 
    else {
        /*
         1、未找到则根据对象的指针创建新的 weak_entry_t。
         2、可能需要扩容weak_table_t中的每个weak_entry_t。
         */
...
weak_clear_no_lock(weak_table_t *weak_table, id referent_id) 
{
    ...
    for (size_t i = 0; i < count; ++i) {
        //使用 DisguisedPtr 定义的运算符重载 *。
        objc_object **referrer = referrers[i];
        if (referrer) {
            if (*referrer == referent) {
                *referrer = nil;    //将该指向该对象的指针的指针置为nil。
            }
            else if (*referrer) {
