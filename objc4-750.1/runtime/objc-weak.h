...
struct weak_entry_t {
    ...
    union {
        struct {
            ...
            uintptr_t        out_of_line_ness : 2;  //对应的值是inline_referrers[1]的低位地址的2位
            uintptr_t        num_refs : PTR_MINUS_2;    //对应的值是inline_referrers[1]的高位地址62位
            uintptr_t        mask;  //用于weak_referrer_t Hash表中地址冲突的辅助常量[线性探测法]
            uintptr_t        max_hash_displacement; //在地址冲突时，可以线性探测的最大次数
        };
        struct {
    ...
    /*
     判断inline_referrers是否够用，因为不够用时，out_of_line_ness = REFERRERS_OUT_OF_LINE;
     */
    bool out_of_line() {
    ...
    /*
     新建，默认先存储在 inline_referrers 数组中。
     */
...
struct weak_table_t {
    ...
    size_t    num_entries;  //弱引用表的 weak_entry_t 实体个数
    uintptr_t mask; //用于实体入口weak_entry_t Hash表中地址冲突的辅助常量[线性探测法]
    uintptr_t max_hash_displacement;    //在地址冲突时，可以线性探测的最大次数
};
