...
static void addNamedClass(Class cls, const char *name, Class replacing = nil)
{
    ...
    Class old;
    /*
     若gdb_objc_realized_classes已存在一个 name 类名，说明在不同 image 模块，都定义了名为 name 的类；
     这种情况是通过保持old class mapping，并将new class尝试插入到 存储没有元类的类的哈希表中
     */
...
/*
 实现了+load的类或分类，会被认定为non-lazy classes，在_read_images预先先加载
 否则会被认为lazy classes，会在使用的时候在进行加载。
 */
/***********************************************************************
* load_images
...
Class readClass(Class cls, bool headerIsBundle, bool headerIsPreoptimized)
{
    ...
    if (headerIsPreoptimized  &&  !replacing) {
        ...
        //向 gdb_objc_realized_classes 按name => cls插入类名为 mangledName 的类
             ...
             bool headerIsPreoptimized, bool headerIsBundle)
{
    ...
    //已进行过预优化(有点类似于 一个协议继承某个协议，但其内部并没有定义新的内容这种情况)
