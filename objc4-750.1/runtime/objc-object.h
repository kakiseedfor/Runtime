...
objc_object::clearDeallocating()
{
    ...
    else if (slowpath(isa.weakly_referenced  ||  isa.has_sidetable_rc)) {   //指针优化情况且有弱引用情况
...
objc_object::rootRetain(bool tryRetain, bool handleOverflow)
{
    ...
    do {
        transcribeToSideTable = false;
        oldisa = LoadExclusive(&isa.bits);  //isa结构体中操作位bits
        newisa = oldisa;
        if (slowpath(!newisa.nonpointer)) { //非指针优化的直接进入sidetable处理
            ClearExclusive(&isa.bits);
        ...
        if (slowpath(tryRetain && newisa.deallocating)) {   //对象正在释放不做处理
            ClearExclusive(&isa.bits);
            if (!tryRetain && sideTableLocked) sidetable_unlock();
            return nil;
        }
        ...
        /*
         Mac上 extra_rc 占用 8bits，最大引用计数为255；
         iPhone extra_rc 占用 19bits，最大应用计数为1048575。
         预先 extra_rc += 1；
         当 extra_rc 达到最大引用计数时，总是把 RC_HALF 放到 SideTable.RefcountMap 中。
         */
        uintptr_t carry;
        ...
        if (slowpath(carry)) {
            ...
            sideTableLocked = true;
            transcribeToSideTable = true;
            newisa.extra_rc = RC_HALF;  //extra_rc = (extra_rc最大应用计数 / 2)
            newisa.has_sidetable_rc = true; //将isa对应的引用计数溢出标志位设为 true
        }
    } while (slowpath(!StoreExclusive(&isa.bits, oldisa.bits, newisa.bits)));   //若isa == oldisa，则isa = newisa，并返回true[猜测应该是为了应对多线程情况]

    /*
     处理引用计数溢出情况。
     将 (extra_rc最大应用计数 / 2) 作为参数传入
     */
...
objc_object::rootRelease(bool performDealloc, bool handleUnderflow)
{
 ...
 retry:
    do {
        ...
        /*
         如果isa.extra_rc = 0，则进入newisa.has_sidetable_rc判断SideTable
         */
        if (slowpath(carry)) {
 ...
 underflow:
    ...
    /*
     newisa.has_sidetable_rc = 1，说明已有部分存在 SideTable.RefcountMap 中。
     */
    if (slowpath(newisa.has_sidetable_rc)) {
        ...
        //从side table中移除 (extra_rc最大应用计数 / 2)
        ...
        if (borrowed > 0) {
            ...
            //继续执行 extra_rc-- [因为之前还未操作]
            newisa.extra_rc = borrowed - 1;  // redo the original decrement too
            //若isa == oldisa，则isa = newisa，并返回true[猜测应该是为了应对多线程情况]
...
objc_object::rootRetainCount()
{
    ...
    if (bits.nonpointer) {
        /*
         [[NSObject alloc] init]的对象，isa.extra_rc = 0。
         因为ARC模式下，有一种优化模式，即：若创建的对象需要被函数返回时，编译器才会默认注入 autorelease 方法。
         */
